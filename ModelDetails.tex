...
For speech recognition community~\cite{yu2012automatic}, a common approach is to adopt the trained GMM-HMM to have the force-alignment labels and use them for the DNNs. Similary we could potentially adopt the same route. However, the contribution to the quality of the label might be trivial considering the increase of the training time. Hence, we argue that the adopted force-alignment scheme will suffice.
...
%
Restricted Boltzmann machine (RBM) are undirected graphical models involving visible and hidden variables,
