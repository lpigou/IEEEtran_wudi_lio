...
The challenge dataset contains 940 videos sequences, each performed by a single person and composed of 10 to 20 gesture instances, totalling about $14,000$ individual gestures.
%
...
are used as validation for monitoring the training performance and the optimisation of the hyper-parameters.
...
$N_u(GT_i, R_i, g)$ reflects the number of frames labelled as a gesture $g$ by  either the ground truth or the prediction,
...
%
Note that experimentally, this measure tends to penalise false positives less than missing true positives.
...
\vspace*{-2mm}
    \caption{
    \small{Results in terms of Jaccard index \jaccardindex for the different network structures and modalities modelling the emission probabilities.}
          }
          \label{Table_score_fusion}
...
%
Fig.~\ref{fig:temporalModellingComparision} visualises the predictions for the different temporal smoothing strategies.

In general, we could observe that better decisions on the presence of gestures and improved localisation of the gesture boundaries are obtained with the proposed \emph{DDNN}.
...
the reuse of pre-trained network parameters, as done in our case, can help to speed up the learning process because the improved initialisation leads to faster convergence.
%
...
Because the multimodal network is being initialised with the individual subnetwork parameters, its training time is only half that of the stand-alone 3DCNN.


\mypartitle{Complexity at test time.}
%
Given the trained models, our framework can perform real-time video sequence labelling on the GPU, thanks to the low cost of inference.
%
More specifically, a single feed forward neural network incurs linear computational time ($\mathcal{O}(T)$). Furthermore, it can be implemented very efficiently on the GPU because because it requires mainly matrix products and convolutional operations.
...
Remarkably. the preprocessing steps take most of the time and an un-optimized version runs already at 25 FPS, while the
 Viterbi decoding  runs at 90 FPS. Hence, with further optimisations the complete system can achieve faster than real-time performance.
